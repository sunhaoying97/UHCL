...
class DecoderLayer2(nn.Module):
    ...
    def forward(self, enc_output, dec_enc_attn_mask=None):

        slf_output, sel_att_scores = self.slf_attn(enc_output, enc_output, enc_output, dec_enc_attn_mask)
        #dec_output, dec_att_scores = self.enc_attn(enc_output, enc_output, enc_output, dec_enc_attn_mask)
        intermediate_output = self.intermediate(slf_output)
        dec_output = self.output(intermediate_output, slf_output)
        return dec_output, sel_att_scores
class Decoder2(nn.Module):
    def __init__(self, config):
        super(Decoder2, self).__init__()
        layer = DecoderLayer2(config)
        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(config.num_decoder_layers)])

    def forward(self, encoder_outs, output_all_encoded_layers=False):
        dec_att_scores = None
        all_encoder_layers = []
        all_dec_att_probs = []
        for layer_module in self.layer:
            dec_output, sel_att_scores = layer_module(encoder_outs)
            if output_all_encoded_layers:
                all_encoder_layers.append(dec_output)
                all_dec_att_probs.append(dec_att_scores)
        if not output_all_encoded_layers:
            all_encoder_layers.append(dec_output)
            all_dec_att_probs.append(dec_att_scores)
...
class DecoderModel(PreTrainedModel):
    ...
    def __init__(self, config, decoder_word_embeddings_weight, decoder_position_embeddings_weight,classify_flag=1):
        ...
        self.decoder2 = Decoder2(config)
        #self.decoder_action = Decoder(config)
        self.classifier = DecoderClassifier(config, decoder_word_embeddings_weight)
        self.apply(self.init_weights)
        self.classify_flag = classify_flag
        if self.config.cluster:
            self.t_ctm0 = CTM(sample_ratio=config.t_rate0, embed_dim=768, dim_out=768, k=3)
            self.t_block0 = TCBlock(dim=768, num_heads=8)
            self.t_ctm1 = CTM(sample_ratio=config.t_rate1, embed_dim=768, dim_out=768, k=3)
            self.t_block1 = TCBlock(dim=768, num_heads=8)

            self.v_ctm0 = CTM(sample_ratio=config.v_rate0, embed_dim=768, dim_out=768, k=3)
            self.v_block0 = TCBlock(dim=768, num_heads=8)
            self.v_ctm1 = CTM(sample_ratio=config.v_rate1, embed_dim=768, dim_out=768, k=3)
            self.v_block1 = TCBlock(dim=768, num_heads=8)

            self.classifier_action = nn.Linear(768, 201)
            self.classifier_entity = nn.Linear(768, 401)
    def forward(self, input_ids, encoder_outs=None, answer_mask=None, encoder_mask=None):
        ...
        #         uniform_noise_ball = torch.randn(input_shape, device=encoder_outs.device)  # normal distribution
        #         uniform_noise_sphere = torch.nn.functional.normalize(uniform_noise_ball, dim=1)
        #         u = torch.rand(input_shape[0], device=encoder_outs.device)  # unified distribution
        #         u = u ** (1. / input_shape[1])
        ...
        #                                  device=encoder_outs.device) * std)  # todo by some conventions multivraiance noise should be devided by sqrt of dim
        #         if modality_offset is not None:
        #             x = x + modality_offset
        #         return torch.nn.functional.normalize(x, dim=1)
        #     encoder_outs = noise_injection(encoder_outs, noise_variance, modality_offset=modality_offset, uniform_noise=uniform_noise, dont_norm=dont_norm)
        ...
        decoded_layers, dec_att_scores = self.decoder(embedding_output,#[64, 48, 768]
                                      encoder_outs, #[64, 20, 768]
                                      self_attn_mask,#[64, 1, 48, 48]
                                      extended_encoder_mask,#[64, 1, 1, 20]             [-1000]
                                      )
        ...
        if self.training:
            if self.config.cluster:
                # t_idx_token = torch.arange(embedding_output.size(1))[None, :].repeat(embedding_output.size(0), 1)
                # t_agg_weight = embedding_output.new_ones(embedding_output.size(0), embedding_output.size(1), 1)
                # t_token_dict = {'x': embedding_output,
                #                 'token_num': embedding_output.size(1),
                #                 'idx_token': t_idx_token,
                #                 'agg_weight': t_agg_weight,
                #                 'mask':answer_mask.detach()}#answer_mask.detach()
                # ##print(embedding_output.shape)
                # t_token_dict = self.t_block0(self.t_ctm0(t_token_dict))
                #embedding_output_t_entiy = t_token_dict["x"]

                v_idx_token = torch.arange(encoder_outs.size(1))[None, :].repeat(encoder_outs.size(0), 1)
                v_agg_weight = encoder_outs.new_ones(encoder_outs.size(0), encoder_outs.size(1), 1)
                v_mask =  encoder_mask
                v_token_dict = {'x': encoder_outs,
                                'token_num': encoder_outs.size(1),
                                'idx_token': v_idx_token,
                                'agg_weight': v_agg_weight,
                                'mask': v_mask.detach()}

                v_token_dict = self.v_block0(self.v_ctm0(v_token_dict))
                encoder_outs_v_entity = v_token_dict["x"]

                #decoded_layers_entity, dec_att_scores_entity = self.decoder2(
                #                                              encoder_outs_v_entity
                #                                              )
                #sequence_output_entity = decoded_layers_entity[-1]
                cls_scores_entity = self.classifier_entity(encoder_outs_v_entity)


                #t_token_dict = self.t_block1(self.t_ctm1(t_token_dict))
                v_token_dict = self.v_block1(self.v_ctm1(v_token_dict))
                #embedding_output_v_action = t_token_dict["x"]
                encoder_outs_v_action = v_token_dict["x"]

                #decoded_layers_action, dec_att_scores_action = self.decoder2(
                #                              encoder_outs_v_action
                #                              )

                #sequence_output_action = decoded_layers_action[-1]
                cls_scores_action = self.classifier_action(encoder_outs_v_action)
                #cands = torch.topk(cls_scores, k=1, dim=-1)
                return cls_scores, sequence_output, cls_scores_entity, cls_scores_action,encoder_outs_v_entity,encoder_outs_v_action
            else:
                return cls_scores, sequence_output
        else:
            return cls_scores, sequence_output

"""
[-0.9731,  0.4235, -0.0618,  ..., -0.3005,  0.8853,  0.1667],
         [-0.0996,  0.8762, -0.9049,  ..., -0.8629,  0.2286,  0.2581],
         [-1.0392,  1.0684, -0.1317,  ..., -0.5666,  0.6407, -0.1952]],


extended_encoder_mask[0]:
      tensor([[[-10000., -10000., -10000., -10000., -10000., -10000., -10000.,
      -10000., -10000., -10000., -10000., -10000., -10000., -10000.,
      -10000., -10000., -10000., -10000., -10000., -10000.]]],device='cuda:0')


self_attn_mask[0]:torch.Size([1, 48, 48])
        tensor([[[    -0., -10000., -10000., -10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000,],
                 [    -0.,     -0., -10000., -10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000., -10000., -10000,...],
                 [    -0.,     -0.,     -0., -10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000., -10000., -10000,...],
                 [    -0.,     -0.,     -0.,     -0.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000., -10000., -10000,...],
                 [    -0.,     -0.,     -0.,     -0.,    -0.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000., -10000., -10000,...],
                 [    -0.,     -0.,     -0.,     -0.,    -0.,    -0.,-10000.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000., -10000., -10000,...],
                 [    -0.,     -0.,     -0.,     -0.,    -0.,    -0.,    -0.,-10000.,-10000.,-10000.,-10000.,-10000., -10000., -10000., -10000., -10000,...],

...
class AllGather(torch.autograd.Function):
    """An autograd function that performs allgather on a tensor.一个在张量上执行allgather操作的自动求导函数""" 
    #在分布式训练中同步不同进程的梯度
    @staticmethod
    def forward(ctx, tensor, args):
        #output:一个与输入张量形状相同的张量列表 #列表的长度为world_size
        output = [torch.empty_like(tensor) for _ in range(args.world_size)] #world_size进程数
        ## 执行allgather操作，将输入张量的内容收集到所有进程中
    ...
    def backward(ctx, grad_output):
        ## 在反向传播中，只返回当前进程对应部分的梯度，其余部分的梯度被丢弃
        return (
